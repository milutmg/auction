...
const getCategoryImage = (categoryName: string): string => {
  const imageMap: { [key: string]: string } = {
    ...
    'Ceramics & Pottery': 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80',
  ...
  return imageMap[categoryName] || 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80';
};
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground mt-2">
                          <span className="text-green-600">{category.active_auction_count} active</span>
                          {parseInt(category.pending_auction_count) > 0 && (
                            <span className="ml-2 text-orange-600">{category.pending_auction_count} pending</span>
                          )}
                        </div>
                        {parseFloat(category.avg_price) > 0 && (
                          <p className="text-xs text-muted-foreground mt-1">
                            Avg: ${parseFloat(category.avg_price).toFixed(0)}
                          </p>
                        )}
                      </div>
                      <img
                        src={getCategoryImage(category.name)}
                        alt={category.name}
                        className="w-14 h-14 rounded-md object-cover ml-3"
                      />
                    </div>
                    <div className="mt-4">
                      <Link
